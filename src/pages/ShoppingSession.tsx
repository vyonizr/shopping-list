  ...
  const handleSaveNote = async (id: number | undefined) => {
    ...
    try {
      if (tempNote.trim()) {
        ...
        const existingNote = await db.sessionNotes
          .where('item_id')
          .equals(id)
          .first();
        ...
        if (existingNote) {
          ...
          await db.sessionNotes.update(existingNote.id!, {
            note: tempNote.trim(),
          });
        } else {
          // Add new note
      ...
      {isQueryLoading ? (
        ...
        <section className="bg-white p-8 sm:p-12 rounded-lg shadow-sm border border-gray-100 text-center">
          ...
          <p className="text-lg text-gray-600 mb-4">
            No items selected for shopping.
          </p>
          <p className="text-sm text-gray-500 mb-6">
            Go to Everyday Items and select items to add them here.
          </p>
          <Button asChild>
            <Link to="/">Go to Everyday Items</Link>
          </Button>
        </section>
      ) : (
        <>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will deselect all items and clear your shopping session. Are
